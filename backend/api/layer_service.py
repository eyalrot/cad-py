...
from backend.models import Color, Layer, LineType
...
class LayerService:
    ...
    def get_layer(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            layer = document.get_layer(layer_id)
            if not layer:
                return {"found": False, "error_message": f"Layer {layer_id} not found"}
            ...
            return {"found": False, "error_message": f"Failed to get layer: {str(e)}"}
    ...
    def list_layers(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return ProtobufConverters.create_success_response(
                {
                    "layers": [
                        ProtobufConverters.layer_to_proto(layer) for layer in layers
                    ]
                }
            )
