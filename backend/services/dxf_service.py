...
class DXFEntityMapper:
    ...
    def cad_to_dxf_entity(
        self, cad_entity: BaseEntity, msp: "Modelspace"
    ) -> Optional["DXFEntity"]:
    ...
    def _convert_line(self, dxf_entity: "DXFEntity", layer_id: str) -> Line:
        ...
        return Line(start=start_point, end=end_point, layer_id=layer_id)
    ...
    def _convert_circle(self, dxf_entity: "DXFEntity", layer_id: str) -> Circle:
        ...
        return Circle(center=center, radius=radius, layer_id=layer_id)
    ...
    def _convert_polyline(
        self, dxf_entity: "DXFEntity", layer_id: str
    ) -> Optional[BaseEntity]:
        ...
        return Line(start=points[0], end=points[1], layer_id=layer_id)
    ...
    def _create_dxf_rectangle(self, rect: Rectangle, msp: "Modelspace") -> "DXFEntity":
        ...
        return msp.add_lwpolyline(
            points=points, close=True, dxfattribs={"layer": rect.layer_id}
        )
...
class DXFService:
    ...
    def import_dxf(
        self, file_path: Union[str, Path], options: Optional[DXFImportOptions] = None
    ) -> DXFImportResult:
        ...
        result = DXFImportResult(
            document=None, success=False, errors=[], warnings=[], stats={}
        )
        ...
        try:
            ...
            logger.info(
                f"DXF import completed: {entity_count} entities, {layer_count} layers"
            )
    ...
    def export_dxf(
        ...
        result = DXFExportResult(
            success=False, file_path=None, errors=[], warnings=[], stats={}
        )
        ...
        try:
            ...
            logger.info(
                f"DXF export completed: {entity_count} entities, {layer_count} layers"
            )
    ...
    def _import_layers(
        ...
        try:
            for dxf_layer in dxf_doc.layers:
                ...
                line_type = self._map_line_type(
                    getattr(dxf_layer.dxf, "linetype", "CONTINUOUS")
                )
                line_weight = (
                    getattr(dxf_layer.dxf, "lineweight", 25) / 100.0
                )  # Convert from 1/100mm to mm
            ...
            for dxf_entity in modelspace:
                ...
                if cad_entity:
                    ...
                    if options.scale_factor != 1.0:
                        cad_entity = self._scale_entity(
                            cad_entity, options.scale_factor
                        )
    ...
    def _export_entities(
        ...
        try:
            ...
            for cad_entity in cad_doc.entities:
                ...
                # Convert entity
                dxf_entity = self.entity_mapper.cad_to_dxf_entity(
                    cad_entity, modelspace
                )
                ...
                if dxf_entity:
                    ...
                    result.warnings.append(
                        f"Failed to convert {type(cad_entity).__name__} entity"
                    )
