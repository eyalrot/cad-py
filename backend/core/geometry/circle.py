...
class Circle:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        tolerance = 1e-10
        return (
            self.center == other.center and abs(self.radius - other.radius) < tolerance
        )
    ...
    def intersection_with_circle(self, other: "Circle") -> List[Point2D]:
        ...
        if (
            abs(d - (self.radius + other.radius)) < 1e-10
            or abs(d - abs(self.radius - other.radius)) < 1e-10
        ):
    ...
    def bounding_box(self) -> Tuple[Point2D, Point2D]:
        ...
        """
        min_point = Point2D(self.center.x - self.radius, self.center.y - self.radius)
        max_point = Point2D(self.center.x + self.radius, self.center.y + self.radius)
    ...
    def from_three_points(cls, p1: Point2D, p2: Point2D, p3: Point2D) -> "Circle":
        ...
        if center is None:
            raise ValueError(
                "Cannot find circle center (perpendicular bisectors are parallel)"
            )
