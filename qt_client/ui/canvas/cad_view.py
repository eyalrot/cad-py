...
class CADView(QGraphicsView):
    ...
    def pan_by(self, delta: QPointF):
        ...
        scene_delta = self.mapToScene(delta.toPoint()) - self.mapToScene(QPointF(0, 0).toPoint())
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MouseButton.MiddleButton or self._space_pressed:
            ...
            for item in items:
                if (item.flags() & QGraphicsItem.GraphicsItemFlag.ItemIsSelectable and
                    item.zValue() <= 900):  # Skip helper items
                    clicked_item = item
                    break
            
            # If not clicking on a selected item, start new selection
            if not clicked_item or not self._selection_manager.is_item_selected(clicked_item):
        ...
        # Handle window selection
            ...
            if delta > 5:  # Minimum drag distance in pixels
                ...
                if not self._selection_manager._is_selecting:
                    self._selection_manager.start_window_selection(self._selection_start_pos)
                else:
