...
class CADScene(QGraphicsScene):
    ...
    def add_cad_item(self, item: QGraphicsItem, item_id: str, layer: str = None) -> bool:
    ...
    def create_layer(self, name: str, color: QColor = None, 
                    line_type: str = "continuous", line_weight: float = 0.25) -> bool:
        """Create a new layer."""
        try:
    ...
    def delete_layer(self, name: str) -> bool:
        ...
        try:
            ...
            if items_on_layer:
                logger.warning(f"Cannot delete layer {name}: has {len(items_on_layer)} items")
                return False
