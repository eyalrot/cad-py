...
from PyQt6.QtWidgets import QApplication, QGraphicsScene, QGraphicsLineItem, QGraphicsEllipseItem, QGraphicsRectItem
from PyQt6.QtGui import QLineF

from qt_client.core.selection_manager import SelectionManager, SelectionMode, SelectionFilter
...
def sample_items(scene):
    ...
    circle.setData(1, "0")
    circle.setFlags(circle.flags() | QGraphicsEllipseItem.GraphicsItemFlag.ItemIsSelectable)
...
class TestSelectionManager:
    ...
    def test_pick_select_ctrl_toggle(self, qapp, selection_manager, sample_items):
        ...
        selection_manager.pick_select(QPointF(50, 50), Qt.KeyboardModifier.ControlModifier)
        assert selection_manager.is_item_selected(line1)
        assert selection_manager.is_item_selected(line2)
        assert selection_manager.get_selection_count() == 2
        
        # Ctrl+click line1 again (should remove from selection)
        selection_manager.pick_select(QPointF(50, 0), Qt.KeyboardModifier.ControlModifier)
        assert not selection_manager.is_item_selected(line1)
        assert selection_manager.is_item_selected(line2)
        assert selection_manager.get_selection_count() == 1
    ...
    def test_pick_select_shift_add(self, qapp, selection_manager, sample_items):
        ...
        selection_manager.pick_select(QPointF(50, 50), Qt.KeyboardModifier.ShiftModifier)
        assert selection_manager.is_item_selected(line1)
        assert selection_manager.is_item_selected(line2)
        assert selection_manager.get_selection_count() == 2
    ...
    def test_invert_selection(self, qapp, selection_manager, sample_items):
        ...
        assert not selection_manager.is_item_selected(line1)
        assert selection_manager.get_selection_count() == len(sample_items) - initial_count
