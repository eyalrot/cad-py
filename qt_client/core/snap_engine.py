...
class SnapMarker:
    ...
    def show_marker(self, scene, snap_point: SnapPoint):
        ...
        style = self.MARKER_STYLES.get(
            snap_point.snap_type, self.MARKER_STYLES[SnapType.NEAREST]
        )
...
class SnapEngine:
    ...
    def enable_snap_type(self, snap_type: SnapType, enabled: bool = True):
        ...
        logger.debug(
            f"Snap type {snap_type.name} {'enabled' if enabled else 'disabled'}"
        )
    ...
    def find_snap_point(
        self, cursor_pos: QPointF, view_scale: float = 1.0
    ) -> Optional[SnapPoint]:
    ...
    def _get_item_snap_points(
        self, item: QGraphicsItem, cursor_pos: QPointF
    ) -> List[SnapPoint]:
    ...
    def _get_line_snap_points(self, line_item: QGraphicsLineItem) -> List[SnapPoint]:
        ...
        if SnapType.MIDPOINT in self.active_snaps:
            ...
            snap_points.append(
                SnapPoint(QPointF(mid_x, mid_y), SnapType.MIDPOINT, line_item)
            )

        return snap_points

    def _get_ellipse_snap_points(
    ...
    def _get_rect_snap_points(self, rect_item: QGraphicsRectItem) -> List[SnapPoint]:
        ...
        if SnapType.CENTER in self.active_snaps:
            snap_points.append(
                SnapPoint(scene_rect.center(), SnapType.CENTER, rect_item)
            )
    ...
    def _find_intersections(
        self, cursor_pos: QPointF, items: List[QGraphicsItem]
    ) -> List[SnapPoint]:
        ...
        for i, line1 in enumerate(lines):
            for line2 in lines[i + 1 :]:
                ...
                if (
                    intersection
                    and intersection.distance_to(cursor_pos) <= self.world_tolerance
                ):
                    intersection_points.append(
                        SnapPoint(intersection, SnapType.INTERSECTION)
                    )

        return intersection_points

    def _line_intersection(
        self, line1: QGraphicsLineItem, line2: QGraphicsLineItem
    ) -> Optional[QPointF]:
        ...
        try:
            ...
            denom = (p1.x() - p2.x()) * (p3.y() - p4.y()) - (p1.y() - p2.y()) * (
                p3.x() - p4.x()
            )
