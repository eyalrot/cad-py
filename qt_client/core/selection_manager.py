...
from PyQt6.QtWidgets import QGraphicsItem, QGraphicsScene, QGraphicsRectItem
...
class SelectionRubberBand:
    ...
    def start_selection(self, scene: QGraphicsScene, start_point: QPointF, is_crossing: bool = False):
...
class SelectionManager(QObject):
    ...
    selection_filter_changed = pyqtSignal(SelectionFilter)
    ...
    def pick_select(self, point: QPointF, modifiers: Qt.KeyboardModifier = Qt.KeyboardModifier.NoModifier):
    ...
    def finish_window_selection(self, modifiers: Qt.KeyboardModifier = Qt.KeyboardModifier.NoModifier) -> bool:
        ...
        if is_crossing:
            ...
            items = self.scene.items(selection_rect, Qt.ItemSelectionMode.IntersectsItemBoundingRect)
        else:
            # Window selection - items completely inside rectangle
            items = self.scene.items(selection_rect, Qt.ItemSelectionMode.ContainsItemBoundingRect)
        
        # Filter items
        valid_items = [item for item in items if self._passes_filter(item) and self._is_selectable(item)]
    ...
    def select_all(self):
        ...
        valid_items = [item for item in all_items if self._passes_filter(item) and self._is_selectable(item)]
    ...
    def select_similar(self, reference_item: QGraphicsItem):
        ...
        for item in all_items:
            if (self._passes_filter(item) and self._is_selectable(item) and
                type(item) == reference_type and
                item.data(1) == reference_layer):
    ...
    def apply_to_selection(self, operation: Callable[[QGraphicsItem], None]):
        ...
        for item in self._selected_items.copy():  # Copy to avoid modification during iteration
            try:
                operation(item)
            except Exception as e:
